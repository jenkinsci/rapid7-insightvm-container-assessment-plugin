...
public class SafeArchiveServingAction extends AssessmentReportBaseAction {
  ...
  public HttpResponse doDynamic(StaplerRequest req, StaplerResponse rsp) throws IOException {
    ...
    if (!file.getAbsolutePath().startsWith(this.getRootDir().getAbsolutePath())) {
      // TODO symlinks and similar things?
      if (LOGGER.isLoggable(Level.FINEST)) {
  ...
  private HttpResponse serveFile(File file) {
    if (safeDirectories != null && !safeDirectories.isEmpty()) {
    ...
    } else if (safeDirectories != null && safeDirectories.contains(file.getParentFile())) {
      ...
      return new DirectoryBrowserSupport(this, new FilePath(rootDir), getDisplayName(), getIconFileName(), false);
    }
  }
